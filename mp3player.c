...
static int PrintFrameInfo(struct mad_header *Header)
{
  ...
  ((MAD_VERSION_MAJOR==0) && (MAD_VERSION_MINOR>=15))
    ...
    if (DEBUG) { printf("%lu kb/s audio MPEG layer %s stream at %dHz\n", Header->bitrate / 1000, Layer, Header->samplerate); }
  ...
  short *_buf = (short *)_buf2;
	...
	while (samplesOut < numSamples) {
		...
		if (MAD_RECOVERABLE(Stream.error)) {
			...
			if (DEBUG) { printf("recoverable frame level error (%s)\n", MadErrorString(&Stream)); }
			sceDisplayWaitVblankStart();
		    }
		    return;	//continue;
		    ...
		    if (DEBUG) { printf("unrecoverable frame level error (%s).\n", MadErrorString(&Stream));}
		    sceDisplayWaitVblankStart();
		    Status = 1;
		    MP3_Stop();	//break;
		}
...
void MP3_FreeTune()
{
	...
	if (DEBUG) { printf("%lu frames decoded (%s).\n", FrameCount, Buffer); }
	sceDisplayWaitVblankStart();
...
int MP3_Load(char *filename)
{
	...
	if (ptr != 0) {		// Read file in
	    ...
	    if (DEBUG) { printf("Error allocing\n"); }
	    sceIoClose(fd);
	    return 0;
	}
	// Close file
